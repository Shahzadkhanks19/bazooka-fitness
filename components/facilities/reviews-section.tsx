  ...
  return (
    <section className="border-b border-bazooka-border/70 bg-bazooka-soft-black py-14 sm:py-16">
      <div className={shell}>
        ...
        <div className="mx-auto mt-8 grid max-w-[1120px] gap-6 lg:grid-cols-[340px_1fr] lg:items-stretch">
          <motion.div {...reveal} className="flex min-h-[290px] flex-col justify-between rounded-[6px] border border-bazooka-border bg-bazooka-black/35 p-7 sm:min-h-[310px] sm:p-8 lg:min-h-0">
            <div>
              ...
              <div className="mt-7 flex items-end gap-3">
                <strong className="font-display text-[58px] leading-none sm:text-[64px]">4.9</strong>
                <span className="pb-1 text-[9px] font-bold uppercase tracking-[.04em] text-bazooka-text-secondary">out of 5</span>
              </div>
              ...
              <p className="mt-5 max-w-[220px] text-[10px] leading-5 text-bazooka-text-secondary">Member feedback on the Bazooka training environment and facilities.</p>
            </div>
            <div className="mt-7 border-t border-bazooka-border/70 pt-5">
              <small className="inline-flex items-center gap-2.5 text-[8px] font-black uppercase tracking-[.08em] text-white"><ShieldCheck className="size-[18px] shrink-0 text-bazooka-lime" /> Google Reviews</small>
            </div>
          </motion.div>

          <motion.article key={reviewIndex} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .35, ease: "easeOut" }} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={.18} onDragEnd={(_, info) => { if (info.offset.x <= -45) nextReview(); if (info.offset.x >= 45) previousReview(); }} className="relative min-h-[310px] touch-pan-y cursor-grab rounded-[6px] border border-bazooka-border bg-[#111214] p-6 pb-16 sm:p-8 sm:pb-16 active:cursor-grabbing">
            <div className="flex items-start justify-between gap-4"><FcGoogle className="size-8 shrink-0" /><div className="flex gap-1 text-[#fbbc04]">{Array.from({ length: 5 }).map((_, index) => <Star key={index} className="size-3.5 fill-current" />)}</div></div>
            <p className="mt-7 max-w-[720px] text-[12px] leading-6 text-bazooka-text-secondary sm:text-[13px]">“{review.text}”</p>
            <div className="mt-8 flex min-w-0 items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-bazooka-surface text-sm font-black text-bazooka-lime">{review.name.charAt(0)}</span><div className="min-w-0"><strong className="block text-[10px]">{review.name}</strong><small className="text-[8px] text-bazooka-text-secondary">{review.meta}</small><span className="mt-1 flex items-center gap-1 text-[7px] text-bazooka-lime"><ShieldCheck className="size-3" /> Google Review</span></div></div>
