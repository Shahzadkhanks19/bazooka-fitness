...
export default function FacilitiesHeroSection() {
  return <section className="border-b border-bazooka-border/70 bg-bazooka-black">
    <div className="relative min-h-[560px] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-black via-black/78 to-black/10" /><div className="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-black/30" />
      <div className={`${shell} relative z-10 flex min-h-[560px] items-center py-10 sm:py-14`}><motion.div {...reveal} className="w-full max-w-[660px]">
        <div className="mb-6 flex flex-wrap items-center gap-2 text-[8px] font-black uppercase sm:text-[9px]"><Link href="/" className="text-white transition hover:text-bazooka-lime">Home</Link><ChevronRight className="size-3 text-bazooka-lime" /><span className="text-bazooka-text-secondary">Facilities</span></div>
        ...
        <h1 className="font-display mt-3 max-w-[720px] text-[40px] font-black uppercase leading-[.88] min-[380px]:text-[45px] sm:text-[64px] lg:text-[82px]">Facilities<br /><span className="text-bazooka-lime">That Inspire.</span></h1>
    ...
    <div className={`${shell} relative z-20 pb-8 pt-4 sm:-mt-7 sm:pt-0`}><div className="grid grid-cols-2 overflow-hidden rounded-[5px] border border-bazooka-border-strong bg-bazooka-soft-black sm:grid-cols-2 lg:grid-cols-5">{stats.map(([Icon,value,label],index)=><motion.div key={String(label)} {...reveal} transition={{duration:.45,delay:index*.04}} className="relative flex min-h-[84px] items-center justify-center gap-3 px-3 py-4 sm:min-h-[92px] sm:gap-4 sm:px-4"><Icon className="size-5 shrink-0 text-bazooka-lime sm:size-6"/><div><strong className="font-display block text-[18px] font-black uppercase leading-none sm:text-[22px]">{value}</strong><small className="mt-1 block text-[7px] font-bold uppercase leading-3 text-bazooka-text-secondary">{label}</small></div>{index<stats.length-1&&<span className="absolute right-0 top-1/2 hidden h-10 w-px -translate-y-1/2 bg-bazooka-border lg:block"/>}</motion.div>)}</div></div>
  </section>;
}
