...
function FloatingAction({label,children,className,href,onClick}:{label:string;children:React.ReactNode;className:string;href?:string;onClick?:()=>void}) {
  ...
  if(href) return <Link href={href} className={shared} aria-label={label}>{children}</Link>;
  return <button type="button" onClick={onClick} className={shared} aria-label={label}>{children}</button>;
}
...
export default function FloatingActions() {
  ...
  return <>
    <aside className="fixed bottom-3 right-3 z-[90] flex flex-col gap-2 sm:bottom-5 sm:right-5 sm:gap-3" aria-label="Quick actions">
      ...
      <a href="https://wa.me/919116405151" target="_blank" rel="noreferrer" className="grid size-11 place-items-center rounded-full bg-bazooka-whatsapp text-white shadow-lg transition-all duration-300 hover:scale-105 active:scale-95 sm:size-13 sm:hover:scale-110" aria-label="WhatsApp"><FaWhatsapp className="size-5 sm:size-6"/></a>
      ...
      <AnimatePresence initial={false}>{showBackToTop&&<motion.div key="back-to-top" initial={{opacity:0,scale:.8,y:8}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.8,y:8}} transition={{duration:.2}}><Link href="#site-top" aria-label="Back To Top" className="group grid size-11 place-items-center rounded-full border border-bazooka-border bg-bazooka-card text-white shadow-lg transition-all duration-300 hover:scale-105 hover:border-bazooka-lime hover:text-bazooka-lime active:scale-95 sm:size-13 sm:hover:scale-110"><ChevronUp className="size-[18px] sm:size-5"/></Link></motion.div>}</AnimatePresence>
    </aside>

    <AnimatePresence>{chatOpen&&<motion.section initial={{opacity:0,scale:.96,y:15}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.96,y:15}} className="fixed bottom-16 left-3 right-[66px] z-[120] flex max-h-[min(70dvh,520px)] flex-col overflow-hidden rounded-xl border border-bazooka-border bg-bazooka-card shadow-2xl sm:bottom-[92px] sm:left-auto sm:right-[82px] sm:w-[min(calc(100%-32px),360px)]"><div className="flex items-center justify-between border-b border-bazooka-border bg-bazooka-surface px-4 py-3"><div><strong className="block text-[11px] text-bazooka-lime">ASK BAZOOKA</strong><span className="text-[8px] text-bazooka-text-muted">Quick website assistant</span></div><button type="button" onClick={()=>setChatOpen(false)} className="grid size-8 place-items-center rounded-full transition hover:bg-bazooka-card-hover active:scale-95" aria-label="Close chat"><X className="size-4"/></button></div><div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.map(item=><div key={item.id} className={`flex ${item.role==="user"?"justify-end":"justify-start"}`}><p className={`max-w-[88%] rounded-[8px] px-3 py-2 text-[10px] leading-5 ${item.role==="user"?"bg-bazooka-lime text-black":"border border-bazooka-border bg-bazooka-black text-bazooka-text-secondary"}`}>{item.text}</p></div>)}</div><div className="flex flex-wrap gap-1.5 border-t border-bazooka-border px-3 pt-3">{["Memberships","Free trial","Trainers","Cafe Bazooka"].map(prompt=><button key={prompt} type="button" onClick={()=>{setMessage(prompt);const now=Date.now();setMessages(current=>[...current,{id:now,role:"user",text:prompt},{id:now+1,role:"assistant",text:getBazookaReply(prompt)}]);}} className="rounded-full border border-bazooka-border px-2.5 py-1.5 text-[8px] text-bazooka-text-secondary transition hover:border-bazooka-lime hover:text-bazooka-lime active:scale-95">{prompt}</button>)}</div><form onSubmit={sendMessage} className="flex gap-2 p-3"><input value={message} onChange={event=>setMessage(event.target.value)} aria-label="Message" placeholder="Type your question..." className="min-w-0 flex-1 rounded-md border border-bazooka-border bg-bazooka-black px-3 py-2 text-[16px] outline-none transition focus:border-bazooka-lime"/><button type="submit" disabled={!message.trim()} className="grid size-10 shrink-0 place-items-center rounded-md bg-bazooka-lime text-black transition hover:bg-bazooka-lime-hover active:scale-95 disabled:cursor-not-allowed disabled:opacity-45" aria-label="Send message"><ArrowRight className="size-4"/></button></form></motion.section>}</AnimatePresence>
  </>;
}
