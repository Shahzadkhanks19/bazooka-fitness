...
function Logo() {
  return <span className="inline-flex min-w-[170px] flex-col leading-none"><span className="text-[30px] font-black tracking-[-1.8px] text-bazooka-lime">BAZOOKA</span><span className="mt-1 text-[5px] tracking-[0.6px] text-bazooka-text-secondary">FITNESS • PERFORMANCE • COMMUNITY</span></span>;
}
...
export default function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-[100] h-[76px] border-b border-white/10 bg-bazooka-black/95 backdrop-blur-xl">
      ...
      {menuOpen && <motion.nav initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="absolute inset-x-0 top-[76px] z-[110] max-h-[calc(100vh-76px)] overflow-y-auto border-b border-bazooka-border bg-bazooka-black/98 px-5 py-5 shadow-2xl backdrop-blur-2xl xl:hidden" aria-label="Mobile navigation"><div className="mx-auto grid max-w-xl gap-1">{nav.map(([label, href]) => { const active = isActive(href); return <Link key={label} href={href} onClick={() => setMenuOpen(false)} className={`rounded-md px-4 py-3.5 text-sm font-bold transition ${active ? "bg-bazooka-lime/10 text-bazooka-lime" : "text-white hover:bg-bazooka-card hover:text-bazooka-lime"}`}>{label}</Link>; })}<Link href="/book-free-trial" onClick={() => setMenuOpen(false)} className={`${trialButton} mt-3 w-full sm:hidden`}>Book Free Trial <ArrowRight className="size-4" /></Link></div></motion.nav>}
    </header>
  );
}
