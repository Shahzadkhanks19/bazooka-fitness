...
export default function SiteHeader() {
  ...
  return <header id="site-top" className="sticky top-0 z-[100] h-[68px] border-b border-white/10 bg-bazooka-black/95 backdrop-blur-xl sm:h-[76px]">
    <div className={`${shell} flex h-full min-w-0 items-center gap-4 sm:gap-7`}>
      <Link href="/" prefetch={false} onClick={() => setMenuOpen(false)} aria-label="Bazooka Fitness home" className="min-w-0 shrink transition-transform duration-300 hover:scale-[1.02]"><Logo /></Link>
      <nav className="hidden flex-1 items-center justify-center gap-4 xl:flex" aria-label="Primary navigation">{nav.map(([label,href]) => <Link key={label} href={href} prefetch={false} aria-current={isActive(href)?"page":undefined} className={`group relative grid h-[76px] place-items-center text-[12px] font-bold transition-colors duration-300 ${isActive(href)?"text-bazooka-lime":"text-bazooka-text hover:text-bazooka-lime"}`}>{label}<span className={`absolute bottom-4 h-[2px] bg-bazooka-lime transition-all duration-300 ${isActive(href)?"w-8":"w-0 group-hover:w-8"}`}/></Link>)}</nav>
      <Link href="/book-free-trial" prefetch={false} className={`${trialButton} ml-auto hidden min-w-[176px] xl:inline-flex`}>Book Free Trial <ArrowRight className="size-4 transition-transform group-hover:translate-x-1"/></Link>
