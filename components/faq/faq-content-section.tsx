...
export default function FaqContentSection({ search, topic, onTopic }: { search: string; topic: FaqTopic | "All"; onTopic: (topic: FaqTopic | "All") => void }) {
  ...
  const renderFaq = (item: (typeof faqItems)[number]) => {
    ...
    return <article key={item.question} className="overflow-hidden rounded-[4px] border border-bazooka-border-strong bg-bazooka-black">
      <button type="button" aria-expanded={open} onClick={()=>setOpenQuestion(open?"":item.question)} className={`flex min-h-[58px] w-full items-center justify-between gap-4 px-4 py-4 text-left text-[10px] font-bold transition hover:text-bazooka-lime ${open?"text-bazooka-lime":"text-white"}`}><span>{item.question}</span>{open?<Minus className="size-4 shrink-0"/>:<Plus className="size-4 shrink-0 text-bazooka-lime"/>}</button>
      {open&&<div className="border-t border-bazooka-border px-4 py-4 text-[9px] leading-5 text-bazooka-text-secondary">{item.answer}</div>}
    </article>;
  };
