...
function HelpPanel({copy}:{copy:string}) {
  return <div className="mt-4 grid items-center gap-5 rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-6 lg:grid-cols-[1.15fr_.75fr_.65fr_.75fr_auto]"><div className="flex gap-4"><span className="grid size-14 shrink-0 place-items-center rounded-full border border-bazooka-lime bg-bazooka-lime/5 text-bazooka-lime"><Headphones className="size-7"/></span><div><h2 className="font-display text-[24px] font-black uppercase">Need Help?</h2><p className="mt-1 max-w-[280px] text-[9px] leading-4 text-bazooka-text-secondary">{copy}</p></div></div><Info icon={MapPin} title="Visit Us" text="Bazooka Fitness · Sardarpura, Jodhpur"/><Info icon={Phone} title="Call Us" text="+91 91164 05151"/><Info icon={Mail} title="Email Us" text="store@bazooka.fit"/><Link href="/contact" className={`${primaryButton} w-full lg:w-auto`}>Contact Us <ArrowRight className="size-4 transition-transform group-hover:translate-x-1"/></Link></div>;
}
