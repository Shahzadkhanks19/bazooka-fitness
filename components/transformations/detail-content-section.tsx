    ...
    <section className="border-b border-bazooka-border/70 bg-bazooka-black py-10 sm:py-12">
      <div className={shell}>
        <motion.div {...reveal} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {story.stats.map((stat,index)=><div key={stat.label} className="group rounded-[5px] border border-bazooka-border-strong bg-bazooka-surface px-5 py-5 transition-all duration-300 hover:border-bazooka-lime/70"><span className="text-[7px] font-black uppercase tracking-[.08em] text-bazooka-text-secondary">0{index + 1}</span><strong className="font-display mt-3 block text-[22px] font-black leading-none text-bazooka-lime sm:text-[25px]">{stat.value}</strong><span className="mt-2 block text-[8px] uppercase leading-4 text-bazooka-text-secondary">{stat.label}</span></div>)}
        </motion.div>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
          <motion.article {...reveal} className="rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-5 sm:p-7">
            <span className="text-[8px] font-black uppercase text-bazooka-lime">How To Approach It</span>
            <h2 className="font-display mt-2 text-[27px] font-black uppercase leading-[.95] sm:text-[32px]">Goal <span className="text-bazooka-lime">Overview</span></h2>
          ...
          <motion.article {...reveal} className="rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-5 sm:p-7">
            <span className="text-[8px] font-black uppercase text-bazooka-lime">Guide Snapshot</span>
            <h2 className="font-display mt-2 text-[27px] font-black uppercase leading-[.95] sm:text-[32px]">Training <span className="text-bazooka-lime">Direction</span></h2>
            <div className="mt-5 divide-y divide-bazooka-border/70">{story.details.map((row)=><div key={row.label} className="grid grid-cols-[1fr_auto] items-center gap-4 py-3 text-[9px]"><span className="text-bazooka-text-secondary">{row.label}</span><strong className="max-w-[190px] text-right leading-4 text-white">{row.value}</strong></div>)}</div>
          </motion.article>
        </div>

        <div className="mt-4 grid gap-4 lg:grid-cols-3">
          {[
            ["Training Direction",story.workoutPlan],
            ["Nutrition Habits",story.nutritionPlan],
        ...
        <p className="mt-5 text-[8px] leading-4 text-bazooka-text-muted">These are general training directions, not medical or nutrition prescriptions. Individual programming should be adapted to the member.</p>
      </div>
    </section>
  );
}
