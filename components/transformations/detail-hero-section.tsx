    ...
    <div className={shell}>
      ...
      <div className="grid gap-7 lg:grid-cols-[360px_1fr] lg:items-stretch">
        <motion.div {...reveal} className="flex min-w-0 flex-col justify-center py-2 sm:py-4">
          ...
          <div className="mt-6 grid grid-cols-1 gap-2 text-[8px] text-bazooka-text-secondary min-[420px]:grid-cols-3 min-[420px]:gap-3">{[[CalendarDays,"Duration",story.duration],[UserRound,"Age",story.age],[MapPin,"Location",story.location]].map(([Icon,label,value])=><div key={String(label)} className="flex items-center gap-3 rounded-[4px] border border-bazooka-border/70 bg-bazooka-surface/60 p-3 min-[420px]:block min-[420px]:rounded-none min-[420px]:border-0 min-[420px]:bg-transparent min-[420px]:p-0"><Icon className="size-4 shrink-0 text-bazooka-lime min-[420px]:mb-2"/><div><span className="block uppercase">{label}</span><strong className="mt-1 block break-words text-[10px] text-white">{value}</strong></div></div>)}</div>
