  ...
  return <>
    ...
    <section className="bg-bazooka-soft-black py-12"><div className={`${shell} grid gap-5 lg:grid-cols-[1fr_320px]`}>
      <form noValidate onSubmit={handleSubmit} className="rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-5 sm:p-6">
        ...
        <textarea aria-label="Specific goals or concerns" className={`${textareaClass} mt-3`} placeholder="Any specific goals or concerns? (Optional)" />
        ...
        <textarea aria-label="Additional information" className={`${textareaClass} mt-5`} placeholder="e.g. Injuries, medical conditions, preferred training style, etc. (Optional)" />
        ...
        {submitted && <p role="status" className="mt-3 rounded-[4px] border border-bazooka-lime/40 bg-bazooka-lime/5 p-3 text-center text-[9px] text-bazooka-lime">Thanks — your session request has been captured. We’ll confirm the selected slot with you.</p>}
      ...
      <aside className="space-y-4"><div className="rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-5"><h2 className="font-display text-[22px] font-black uppercase">Why Train With {firstName}?</h2><div className="mt-5 space-y-4 text-[10px] text-bazooka-text-secondary">{whyItems.map(item => <p key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-bazooka-lime" />{item}</p>)}</div></div><div className="rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-5"><h2 className="font-display text-[20px] font-black uppercase">Need Help?</h2><p className="mt-2 text-[9px] leading-4 text-bazooka-text-secondary">Call our team if you need help choosing a session.</p><a href="tel:+919116405151" className="mt-4 inline-flex items-center gap-2 text-[10px] font-black text-bazooka-lime"><Phone className="size-4" />+91 91164 05151</a></div></aside>
    </div></section>
  </>;
}

function SectionNumber({ n, title, text }: { n: string; title: string; text: string }) { return <div className="flex items-start gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-bazooka-lime text-[10px] font-black text-black">{n}</span><div><h2 className="font-display text-[22px] font-black uppercase">{title}</h2><p className="mt-1 text-[9px] text-bazooka-text-secondary">{text}</p></div></div>; }
