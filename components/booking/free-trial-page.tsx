...
function isSunday(value:string){return !!value&&dateFromValue(value).getDay()===0;}
...
function CustomSelect({label,value,options,onChange}:{label:string;value:string;options:string[];onChange:(value:string)=>void}){
  ...
  return <div className="relative">
    <button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open} className={`${fieldClass} flex items-center justify-between text-left ${value?"text-white":"text-bazooka-muted"}`}>
      <span>{value||label}</span><ChevronDown className={`size-4 text-bazooka-lime transition ${open?"rotate-180":""}`}/>
    </button>
    {open&&<div className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 overflow-hidden rounded-[5px] border border-bazooka-border-strong bg-bazooka-card shadow-2xl">
      {options.map(option=><button key={option} type="button" onClick={()=>{onChange(option);setOpen(false)}} className={`flex w-full items-center justify-between px-4 py-3 text-left text-[10px] transition hover:bg-bazooka-lime/10 hover:text-bazooka-lime ${value===option?"bg-bazooka-lime/10 text-bazooka-lime":"text-white"}`}><span>{option}</span>{value===option&&<Check className="size-4"/>}</button>)}
    </div>}
  </div>;
}
...
function CustomDatePicker({value,onChange,minDate}:{value:string;onChange:(value:string)=>void;minDate:string}){
  ...
  return <div className="relative">
    ...
    {open&&<div className="absolute left-0 top-[calc(100%+6px)] z-50 w-[min(330px,calc(100vw-48px))] rounded-[7px] border border-bazooka-border-strong bg-bazooka-card p-4 shadow-2xl">
      <div className="flex items-center justify-between">
        <button type="button" disabled={!canPrev} onClick={()=>canPrev&&setView(new Date(view.getFullYear(),view.getMonth()-1,1))} className="grid size-9 place-items-center rounded-md border border-bazooka-border text-white transition hover:border-bazooka-lime hover:text-bazooka-lime disabled:cursor-not-allowed disabled:opacity-25"><ChevronLeft className="size-4"/></button>
        <strong className="font-display text-[15px] uppercase">{view.toLocaleDateString("en-IN",{month:"long",year:"numeric"})}</strong>
        <button type="button" onClick={()=>setView(new Date(view.getFullYear(),view.getMonth()+1,1))} className="grid size-9 place-items-center rounded-md border border-bazooka-border text-white transition hover:border-bazooka-lime hover:text-bazooka-lime"><ChevronRight className="size-4"/></button>
      </div>
      <div className="mt-4 grid grid-cols-7 gap-1">{weekDays.map(day=><span key={day} className="py-1 text-center text-[7px] font-black text-bazooka-muted">{day}</span>)}{cells.map((date,index)=>{if(!date)return <span key={`empty-${index}`} className="size-9"/>;const dateValue=localDateValue(date);const blocked=dateValue<minDate||date.getDay()===0;const selected=dateValue===value;return <button key={dateValue} type="button" disabled={blocked} onClick={()=>{onChange(dateValue);setOpen(false)}} className={`grid size-9 place-items-center rounded-md text-[9px] font-bold transition ${selected?"bg-bazooka-lime text-black":"border border-transparent text-white hover:border-bazooka-lime hover:text-bazooka-lime"} disabled:cursor-not-allowed disabled:text-bazooka-disabled disabled:line-through`}>{date.getDate()}</button>})}</div>
...
function TimePicker({date,value,onChange,minDate,currentMinutes}:{date:string;value:string;onChange:(value:string)=>void;minDate:string;currentMinutes:number}){
  ...
  return <div className="relative">
    <button type="button" disabled={!date} onClick={()=>date&&setOpen(v=>!v)} aria-expanded={open} className={`${fieldClass} flex items-center justify-between text-left ${!date?"cursor-not-allowed opacity-50":value?"text-white":"text-bazooka-muted"}`}>
      <span>{value||(!date?"Choose a date first":"Choose preferred time")}</span><Clock3 className="size-4 text-bazooka-lime"/>
    </button>
    {open&&<div className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 max-h-72 overflow-y-auto rounded-[6px] border border-bazooka-border-strong bg-bazooka-card p-2 shadow-2xl">
      <div className="grid gap-1 sm:grid-cols-2">{trialTimes.map(slot=>{const blocked=isToday&&slot.startMinutes<=currentMinutes;return <button key={slot.label} type="button" disabled={blocked} onClick={()=>{onChange(slot.label);setOpen(false)}} className={`rounded-[4px] border px-3 py-3 text-left text-[9px] transition ${value===slot.label?"border-bazooka-lime bg-bazooka-lime/10 text-bazooka-lime":"border-bazooka-border bg-bazooka-black text-white hover:border-bazooka-lime hover:text-bazooka-lime"} disabled:cursor-not-allowed disabled:opacity-30 disabled:line-through`}>{slot.label}</button>})}</div>
    </div>}
  </div>;
}
...
export default function FreeTrialPage(){
  ...
  return <>
    ...
    <section className="bg-bazooka-soft-black py-12"><div className={shell}><div className="text-center"><h2 className="font-display text-[38px] font-black uppercase">Book Your <span className="text-bazooka-lime">Free Trial</span></h2><p className="mt-2 text-[10px] text-bazooka-text-secondary">Fill in your details and we’ll get in touch with you to confirm your trial.</p></div><form noValidate onSubmit={(e)=>{e.preventDefault();const form=e.currentTarget;const name=(form.elements.namedItem("fullName") as HTMLInputElement)?.value.trim();const phone=(form.elements.namedItem("phone") as HTMLInputElement)?.value.trim();const email=(form.elements.namedItem("email") as HTMLInputElement)?.value.trim();if(!name||!phone||!email||!goal||!preferredDate||!preferredTime||!termsAccepted){setSubmitted(false);setFormError("Please complete all required fields before booking your free trial.");return;}setFormError("");setSubmitted(true)}} className="mt-7 rounded-[6px] border border-bazooka-border-strong bg-bazooka-surface p-5 sm:p-7"><div className="grid gap-7 lg:grid-cols-2"><div><h3 className="font-display flex items-center gap-2 text-[20px] font-black uppercase"><UserRound className="size-5 text-bazooka-lime"/>Personal <span className="text-bazooka-lime">Information</span></h3><div className="mt-5 space-y-3"><input name="fullName" className={fieldClass} placeholder="Full Name *"/><input name="phone" className={fieldClass} placeholder="Phone Number *" inputMode="tel"/><input name="email" type="email" className={fieldClass} placeholder="Email Address *"/><div className="grid gap-3 sm:grid-cols-2"><input className={fieldClass} placeholder="Age" inputMode="numeric"/><CustomSelect label="Gender" value={gender} options={["Male","Female","Prefer not to say"]} onChange={setGender}/></div><CustomSelect label="Fitness Goal *" value={goal} options={["Fat Loss","Muscle Gain","Strength","General Fitness"]} onChange={setGoal}/><CustomSelect label="How did you hear about us?" value={source} options={["Google","Instagram","Friend / Family","Walk-in","Other"]} onChange={setSource}/></div></div><div><h3 className="font-display flex items-center gap-2 text-[20px] font-black uppercase"><Target className="size-5 text-bazooka-lime"/>Preferred Trial <span className="text-bazooka-lime">Details</span></h3><div className="mt-5 grid gap-3 sm:grid-cols-2"><CustomDatePicker value={preferredDate} onChange={chooseDate} minDate={minDate}/><TimePicker date={preferredDate} value={preferredTime} onChange={(value)=>{setSubmitted(false);setPreferredTime(value)}} minDate={minDate} currentMinutes={currentMinutes}/></div><p className="mt-2 text-[8px] leading-4 text-bazooka-muted">Open Monday–Saturday, 5:30 AM–10:00 PM. Sunday closed. Time slots are every 30 minutes.</p>{preferredDate===minDate&&!hasAvailableSlotsToday&&<p className="mt-3 rounded-[4px] border border-bazooka-warning/40 bg-bazooka-warning/5 p-3 text-[9px] text-bazooka-warning">Today’s trial slots have ended. Please choose a future date.</p>}<textarea className={`${textareaClass} mt-3`} placeholder="Any specific goals or areas you’d like us to focus on?"/><div className="mt-4 flex items-start gap-3 text-[9px] text-bazooka-text-secondary"><button type="button" onClick={()=>setTermsAccepted(v=>!v)} aria-pressed={termsAccepted} aria-label="Accept terms and privacy policy" className={`mt-0.5 grid size-5 shrink-0 place-items-center rounded-[4px] border transition ${termsAccepted?"border-bazooka-lime bg-bazooka-lime text-black":"border-bazooka-border-strong bg-bazooka-black text-transparent hover:border-bazooka-lime"}`}><Check className="size-3"/></button><span>I agree to the <Link href="/terms-and-conditions" className="text-bazooka-lime">Terms & Conditions</Link> and <Link href="/privacy-policy" className="text-bazooka-lime">Privacy Policy</Link>.</span></div>{formError&&<p className="mt-4 rounded-[4px] border border-bazooka-error/40 bg-bazooka-error/5 p-3 text-center text-[9px] text-bazooka-error">{formError}</p>}<button type="submit" className={`${primaryButton} mt-5 w-full`}>Book My Free Trial <ArrowRight className="size-4"/><Send className="size-4"/></button>{submitted&&<p className="mt-3 rounded-[4px] border border-bazooka-lime/40 bg-bazooka-lime/5 p-3 text-center text-[9px] text-bazooka-lime">Thanks — your free trial request has been captured. Our team will confirm it with you.</p>}</div></div><div className="mt-7 grid gap-4 border-t border-bazooka-border pt-6 sm:grid-cols-3">{trustItems.map(([Icon,a,b])=><div key={a} className="flex items-center justify-center gap-3"><Icon className="size-6 text-bazooka-lime"/><div><strong className="font-display block text-[15px] uppercase">{a}</strong><span className="text-[8px] text-bazooka-text-secondary">{b}</span></div></div>)}</div></form></div></section>
